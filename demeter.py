...
def turn(grid, player_team):
    ...
    while True:
        ...
        if new_line is not None and new_column is not None:  # Si le joueur a selectionné un déplacement
            # S'il ne s'agissait pas d'un saut, tour suivant
            if not is_jump(line, column, new_line, new_column, player_team, grid) or not is_able_to_play(
                ...
                break
            else:  # Sinon, on collecte tous les déplacements possibles avec ce même pion
                next_move_list = ia_test_move((new_line, new_column), player_team, grid, jumped=True)
                turn_ia_remove_last_move(next_move_list, line, column)  # On retire le dernier déplacement de la liste

                if next_move_list != []:  # Si la liste n'est pas vide, lance l'enchainement
                    print("\n*** Vous pouvez jouer à nouveau ***\n")
                    play_again = True
                    line, column = new_line, new_column
                else:  # Sinon, passe au prochain tour
                    unhighlight_pawn(new_line, new_column, grid, player_team)
                    break

        elif not play_again:  # Si le joueur n'a pas encore joué, fait retourner la grille à son état initial
            unhighlight_pawn(line, column, grid, player_team)
        if play_again:  # Sinon, propose au joueur de passer son tour ou d'effectuer une nouvelle saisie
            print("\n*** Voulez vous passer votre tour ? (o/n) ***")
            choice = str(input()).lower()
            if choice == "o":
                unhighlight_pawn(line, column, grid, player_team)
                break

    return check_is_the_end(new_line, new_column, player_team)  # Vérifie si la partie est terminée
...
def turn_move(line, column, player_team, grid, play_again):
    while True:
        new_line, new_column = player_move(line, column, player_team, grid, player_team, play_again)
        if new_line is not None and new_column is not None:  # Modifie la grille et l'affiche
...
def player_move(selected_x, selected_y, selected_team, grid, team, play_again):
    while True:
        print("\n*** Déplacez votre pion (déplacez vous sur vous même pour annuler) ***\n")
        destination_line, destination_column = enter_coordinates()
        if check_move(selected_x, selected_y, selected_team, grid, destination_line, destination_column, team,
                      play_again):
            return destination_line, destination_column
        else:
            print("\n*** Voulez vous effectuer une autre saisie de déplacement pour ce pion ? (o/n) ***\n")
            choice = str(input()).lower()
            if choice == "n":
                return None, None
...
def check_move(selected_x, selected_y, selected_team, grid, destination_line, destination_column, team,
               play_again=False, bot=False):
    if is_different(selected_x, selected_y, destination_line, destination_column):
        if not is_a_pawn(destination_line, destination_column, grid):
            if not is_own_goal(destination_line, destination_column, team):
                ...
                print("\n/!/ Vous ne pouvez pas vous déplacer dans vos goals /!/\n")
        elif not bot:
            print("\n/!/ Vous ne pouvez pas vous déplacer sur un pion /!/\n")
    elif not bot:
...
def is_able_to_play(line, column, team, grid):
    move_list = [[line + 1, column], [line - 1, column], [line, column - 1], [line, column + 1], [line + 2, column + 2],
                 [line + 2, column - 2], [line - 2, column - 2], [line - 2, column + 2], [line - 2, column],
                 [line, column - 2],
    ...
    for move in move_list:
        ...
        if is_on_grid(move_line, move_column):
            if check_move(line, column, team, grid, move_line, move_column, team, bot=True):
                return True
    return False
...
def player_versus_ia(difficulty=0):
    ...
    while True:
        ...
        print("\n\n\n*** La partie a duré", turn_count , "tours ***\n\n\n")
...
def ia_test_move(pawn_coord, team, grid, jumped=False):
    ...
    if not jumped:
        ...
        possible_movement = [[x + 2, y + 2], [x - 2, y + 2], [x - 2, y - 2], [x + 2, y - 2], [x + 2, y], [x - 2, y],
                             [x, y - 2], [x, y + 2]]
    ...
    for movement in possible_movement:
        ...
        if is_on_grid(new_x, new_y):
            if not jumped:
                if check_move(x, y, team, grid, new_x, new_y, team, bot=True):
...
# Choisit aléatoirement un pion dans le movepool
...
# Choisit aléatoirement le mouvement en fonction du pion
...
# Modifie la grille et l'affiche
...
# Retire le dernier mouvement effectué par l'IA de la liste des mouvements
...
# Choisit aléatoirement si l'IA rejoue ou non
def turn_ia_jump_again(next_move_list):
    if next_move_list != []:
...
# Cherche un pion et un mouvement de ce pion pour l'IA naïve, puis modifie la grille en conséquence
...
# Execute un tour pour l'IA naïve
def turn_ia(grid, team, player=False):
    ...
    while True:
        ...
        # Récupère les informations (pion et déplacement) choisies par l'IA
        ia_pawn_line, ia_pawn_column, ia_move_line, ia_move_column = turn_ia_random(movepool, grid, team)

        # Si le mouvement est un saut et que le jeu n'est pas terminé...
        if is_jump(ia_pawn_line, ia_pawn_column, ia_move_line, ia_move_column, team, grid) and not \
            ...
            # On récupère la liste des prochains déplacements de l'IA avec le même pion (uniquement les sauts)
            ...
            # Si l'IA ne décide pas (aléatoirement) de sauter de nouveau...
            if not turn_ia_jump_again(next_move_list):
                break
            else:  # Sinon
                movepool = [[ia_move_line, ia_move_column, next_move_list]]
                print("\n*** L'IA va resauter ***\n")
        else:  # Si le déplacement n'était pas un saut
            break
    ...
    return check_is_the_end(ia_move_line, ia_move_column, team)  # Vérifie si le jeu est terminé


# Execute un tour pour l'IA avancée
def turn_advanced_ia(grid, team, player=False):
    ...
    while True:
        ...
        best_move = ia_find_best_move(movepool, team)  # Trouve le meilleur déplacement possible

        if jumped and not best_move:  # Si l'IA ne trouve pas de meilleur mouvement et qu'elle a sauté, passe son tour
            break
        elif not jumped and not best_move:  # Si aucun meilleur mouvement n'est trouvé, effectue un tour aléatoire
            turn_ia_random(ia_movepool(team, grid, jumped), grid, team)
            break
        elif jumped and best_move:  # Effectue un nouveau saut si l'IA a déjà sauté
            print("\n*** L'IA va resauter ***\n")
        ...
        # Si le déplacement est un saut et que la partie n'est pas terminée...
        if is_jump(ia_pawn_line, ia_pawn_column, ia_move_line, ia_move_column, team, grid) and not \
                is_the_end(ia_move_line, ia_move_column, team):
            jumped = True
            ...
            if next_move_list != []:
                movepool = [[ia_move_line, ia_move_column, next_move_list]]
            else:  # Si on ne trouve pas de mouvements
                break
    ...
    return check_is_the_end(ia_move_line, ia_move_column, team)  # Vérifie si la partie est terminée
...
# Renvoie le meilleur pion à déplacer ainsi que le meilleur déplacement possible de ce pion
def ia_find_best_move(movepool, team):
    goals_position = ia_find_goals(team)
    move_score_list = []

    for i in range(len(movepool)):  # Répertorie tous les mouvements qui ont un score < au score du pion de base
    ...
    for i in range(len(move_score_list)):  # Trie la liste des mouvements dans l'ordre croissant
    ...
    if move_score_list != []:  # Renvoie l'élément classé en premier dans la liste
        return [move_score_list[0][0], move_score_list[0][1], [move_score_list[0][2][0], move_score_list[0][2][1]]]


# Défini le score des mouvements
# Le score fonctionne de la façon suivante : + on est proche du but adverse, plus le score est bas
def ia_find_score(move_list, goals_position, pawn_x, pawn_y):
    move_score_list = []
    i_move = 0
    pawn_score = ia_give_score(pawn_x, pawn_y, goals_position)

    # On crée une deuxième liste qui correspond aux scores de la liste des mouvements
    while len(move_list) > len(move_score_list):
        ...
        # Si un mouvement a un score supérieur au pion actuel, il est supprimé, sinon on le met dans la liste des scores
...
# Attribue un score à un mouvement
def ia_give_score(x, y, goals_position):
    ...
    if y >= 0 and y < 4:  # Utilisation du goal droit ou gauche en fonction du placement du mouvement / du pion
        return abs(goal1_x - x) + abs(goal1_y - y)
    else:
        return abs(goal2_x - x) + abs(goal2_y - y)


# Cherche le mouvement ayant obtenu le meilleur score
def ia_find_best_score(move_list, move_score_list):
    ...
    # Si au moins deux des premiers élements ont un score égal...
    if len(move_score_list) > 1:
        if move_score_list[0] == move_score_list[1]:
            return ia_randomize_best_score(move_score_list, move_list)

    return [move_list[0][0], move_list[0][1], move_score_list[0]]


# Attribue un score aléatoire aux mouvements
def ia_randomize_best_score(move_score_list, move_list):
    best_score = move_score_list[0]
    move_score_list_length = len(move_score_list)

    # Supprime les déplacements dont les scores ne sont pas égaux au plus petit
    for i_move in range(move_score_list_length):
        reverse_i_move = move_score_list_length - 1 - i_move
        if move_score_list[reverse_i_move] != best_score:
            del move_score_list[reverse_i_move]
            del move_list[reverse_i_move]

    # Remplace les scores par des nombres aléatoires
    for i_score in range(len(move_score_list)):
        move_score_list[i_score] = random.randint(0, 10)
...
# Affiche le menu Joueur contre IA
...
# Affiche le menu IA contre IA (debug)
